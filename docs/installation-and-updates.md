...
On upgrades, the startup checkbox reflects the current app setting, even if it differs from the previous installer selection. Silent updates preserve that setting. Unchecking it in Setup removes the startup entry. For scripted installs, `/MERGETASKS=startwithwindows` enables it, `/MERGETASKS=!startwithwindows` disables it, and an explicit `/TASKS=` list or `/LOADINF` file takes precedence over the existing preference.
